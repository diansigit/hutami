...
class Tribe_Image_Widget extends WP_Widget {
	...
	function Tribe_Image_Widget() {
		define('IMAGE_WIDGET_URI',THEME_PLUGINS_URI.'/text-image-widget/');
		load_plugin_textdomain( 'image_widget', false, trailingslashit(basename(dirname(__FILE__))) . 'lang/');
		$widget_ops = array( 'classname' => 'widget_sp_image', 'description' => __( 'Showcase a single image with a Title, and a Description', 'image_widget' ) );
		$control_ops = array( 'id_base' => 'widget_sp_image' );
		$this->WP_Widget('widget_sp_image', __('Text Image Widget', 'image_widget'), $widget_ops, $control_ops);
	...
	function admin_setup() {
		wp_enqueue_media();
		wp_enqueue_script( 'tribe-image-widget', IMAGE_WIDGET_URI.'resources/js/image-widget.js', array( 'jquery', 'media-upload', 'media-views' ), self::VERSION );
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['width'] = 287;//abs( $new_instance['width'] );
		$instance['height'] = 162;//abs( $new_instance['height'] );
		if ( !defined( 'IMAGE_WIDGET_COMPATIBILITY_TEST' ) ) {
	...
	private function get_image_html( $instance, $include_link = true ) {
		...
		if ( $include_link && !empty( $instance['link'] ) ) {
			$attr = array(
				...
				'class' => 	$this->widget_options['classname'].'-image-link thumb-has-overlay',
		...
		if ( is_array( $size ) ) {
			$instance['width'] = 287;//$size[0];
			$instance['height'] = 162;//$size[1];
		...
		$instance['width'] = 287;//abs( $instance['width'] );
		$instance['height'] = 162;//abs( $instance['height'] );
		...
		if ( $include_link && !empty( $instance['link'] ) ) {
			$output .= '<div class="thumb-overlay"></div></a>';
		}
	...
	public function plugin_row_meta( $meta, $file ) {
		if ( $file == IMAGE_WIDGET_URI.'/image-widget.php' ) {
			$meta[] = '<span class="tribe-test">'.sprintf(__('Check out our other <a href="%s" target="_blank">plugins</a> including our <a href="%s" target="_blank">Events Calendar Pro</a>!', 'image_widget'),'http://tri.be/products/?source=image-widget&pos=pluginlist','http://tri.be/wordpress-events-calendar-pro/?source=image-widget&pos=pluginlist').'</span>';
		}
		return $meta;
	}
}
