...
function wp_pagenavi($before = '', $after = '') {
	...
	$pagenavi_options = array(
		...
		'next_text' => __('<i class="fa fa-chevron-right"></i>','theme_front'),
		'prev_text' => __('<i class="fa fa-chevron-left"></i>','theme_front'),
			...
			case 1:
				...
				foreach($larger_pages_array as $larger_page) {
					if ($larger_page < $start_page && $larger_page_start < $larger_page_to_show) {
						...
						echo '<a href="'.esc_url(get_pagenum_link($larger_page)).'" class="page btn" title="'.$page_text.'">'.$page_text.'</a>';
						$larger_page_start++;
					}
				}
				...
				for($i = $start_page; $i  <= $end_page; $i++) {						
					if ($i == $paged) {
						...
						echo '<span class="current btn">'.$current_page_text.'</span>';
					} else {
						$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['page_text']);
						echo '<a href="'.esc_url(get_pagenum_link($i)).'" class="page btn" title="'.$page_text.'">'.$page_text.'</a>';
					}
				}
